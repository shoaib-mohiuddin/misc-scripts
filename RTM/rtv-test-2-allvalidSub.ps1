...
$routeTableGroupedBySub = $routeTableids | Group-Object { $_.split("/")[2] }  # Group route tables by subscription
foreach ($subGroup in $routeTableGroupedBySub) {
  Select-AzSubscription -SubscriptionId $subGroup.Name               # Switch once per subscription
  $subscription = Get-AzSubscription -SubscriptionId $subGroup.Name  # To get sub name in report

  foreach ($routeTableid in $subGroup.Group) {
...
$emailAttachment = @(
  @{
    ...
    Name = "RouteTableComplianceReport-$date.csv"
  }
)
$message = @{
