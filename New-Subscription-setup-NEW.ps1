...
### Partner Admin Link (PAL) to Eviden ID
$PartnerId = Get-AzAutomationVariable -Name 'PartnerId'
#$PartnerId = "6738974"
...
if ($partnerInfo -and $partnerInfo.PartnerId -eq $PartnerId) {
    Write-Output "Partner ID $PartnerId already exists."
} 

else {
    Write-Output "Partner ID $PartnerId does not exist. Adding the partner ID..."
    # Add the partner ID
    New-AzManagementPartner -PartnerId $PartnerId
    Write-Output "Partner ID $PartnerId has been added."
}
###
